...
## bundled 参考 vs SIM（基线，官方口径）

bundled `_fluoresfm` 参考自身对 SIM 真值的质量基线（15 图，SIM 降采样到 502 对齐输出分辨率，每图 P3/P99.5 → clip[0,2.5] → data_range=2.5）：

| 对比 | PSNR | SSIM |
| --- | --- | --- |
| **bundled 参考 vs SIM** | **34.92 dB**（范围 32.8–36.6）| **0.937**（范围 0.878–0.956）|
| 生产候选 vs SIM（推理实验-02）| 35.15 dB | — |
| WF 输入 vs SIM | 27.84 dB | — |

- bundled 参考 vs SIM 平均 **34.92 dB / 0.937**。
- **生产候选 vs SIM（35.15 dB）略高于 bundled（34.92 dB）**—— 我们的输出比参考还稍接近 SIM 真值。
- WF 输入基线 27.84 dB；bundled 参考与生产候选均较输入改善约 +7 dB，确认模型输出去卷积/恢复有效。
