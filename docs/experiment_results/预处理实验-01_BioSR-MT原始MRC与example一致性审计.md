...
以下步骤由审计逐像素验证（0 或 2.4e-7 容差内），但**作者未提供独立官方代码或文档**，且无法通过外部材料获取，作为不可解决边界记录：

1. **9 帧算术均值 → float32 转换**：官方 `read_mrc.py`（BioTISR 补充材料）只读取 MRC 数组（含 `order='F'` 重塑），不含「9 帧均值」步骤；该规则由审计反推并以 0 误差验证。
2. **训练 patch 网格/步长**（32×32→64×64、64×64→128×128，步长=尺寸非重叠）：论文确认 64×64 与 P3/P99.5，但网格生成代码未随仓库发布；审计以 2.4e-7 误差验证。
3. **去噪任务 GT**：example 无干净 WF 参考，论文未定义去噪的「高质量」目标；不影响已冻结的去卷积生产链。
