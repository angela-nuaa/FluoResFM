# 推理实验-03｜官方推理实现与当前实现对比（待确认项清单）

> 日期：2026-08-07（§5 确认实验 2026-08-08 完成）。目的：搜寻官方推理实现（上游 `qiqi-lu/fluoresfm` 仓库），与当前生产实现（napari-fluoresfm 插件 + 生产配置）逐环节对比，列出**需要进一步确定的部分**并逐项确认（全部 7 项已闭环）。
> 指标口径：官方口径（`4_0_result_evaluate.py`）：每图 P3/P99.5 → clip[0,2.5] → PSNR/SSIM data_range=2.5。
...
### 项目 5（论文协议确认）——论文主协议 = 完整 8 字段文本 + P3/P99.5 归一化，无背景扣除
...
### 项目 5 补充：全级别（1/2/4/5/7）复现性验证（2026-08-08）
...
### 项目 5 补充：批量生产执行与 SIM 锚定等价验证（2026-08-08）
